...
if (isset($_POST['toggle_status'])) {
    ...
    // Toggle the status (if it's 'active', change to 'inactive', and vice versa)
    $new_status = ($current_status == 'active') ? 'inactive' : 'active';  

    // Update the status in the database
    $sql = "UPDATE restaurant SET status='$new_status' WHERE rs_id='$restaurant_id'";

    if (mysqli_query($db, $sql)) {
        // Redirect back to the restaurant page after successful status update
...
<body class="fix-header fix-sidebar">
    ...
    <div id="main-wrapper">
        ...
        <div class="page-wrapper">
            ...
            <div class="row page-titles">
                ...
                <h3 class="text-primary"><a href="dashboard.php">Dashboard ></a> Restaurant</h3> </div>
            </div>
            <!-- End Bread crumb -->
            <!-- Container fluid  -->
    ...
    <div class="col-12">
        <div class="card">
            <div class="card-body">
                <h4 class="card-title">All Restaurant data</h4>
                    ...
                    <table id="example23" class="display nowrap table table-hover table-striped table-bordered" cellspacing="0" width="100%">
                        ...
                        <tbody>
                            ...
                            if (!mysqli_num_rows($query) > 0) {
                                echo '<td colspan="12"><center>No restaurant Data!</center></td>';
                            } else {
                                while ($rows = mysqli_fetch_array($query)) {
                                    // Display the category data (if needed)
                                    $mql = "SELECT * FROM food_category WHERE c_id='" . $rows['c_name'] . "'";
                                    $res = mysqli_query($db, $mql);
                                    $row = mysqli_fetch_array($res);

                                    // Set status button properties
                                    $statusBtnClass = $rows['status'] == 'active' ? 'btn-success' : 'btn-danger';
                                    $statusText = $rows['status'] == 'active' ? 'Active' : 'Inactive';
                                    $statusAction = $rows['status'] == 'active' ? 'deactivate' : 'activate';
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                ...
                                                <form method="POST" action="">
                                                    <input type="hidden" name="restaurant_id" value="' . $rows['rs_id'] . '">
                                                    <input type="hidden" name="current_status" value="' . $rows['status'] . '">
                                                    <button type="submit" name="toggle_status" class="btn ' . $statusBtnClass . ' btn-sm m-b-10">
                                                        <i class="fa fa-toggle-on"></i> ' . $statusText . '
                                                    </button>
                                                </form>
                                            </td>
                                            <td>
                                                <a href="delete_stores.php?res_del=' . $rows['rs_id'] . '" class="btn btn-danger btn-flat btn-addon btn-xs m-b-10"><i class="fa fa-trash-o" style="font-size:16px"></i></a>
                                            </td>
                                        </tr>';
                                }
                            }
